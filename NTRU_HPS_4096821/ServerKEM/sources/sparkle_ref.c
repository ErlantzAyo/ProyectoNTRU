...
// 4-round ARX-box
#define ARXBOX(x, y, c)                                                      \
  (x) += ROT((y), 31), (y) ^= ROT((x), 24), (x) ^= (c), (x) += ROT((y), 17), \
      (y) ^= ROT((x), 17), (x) ^= (c), (x) += (y), (y) ^= ROT((x), 31),      \
      (x) ^= (c), (x) += ROT((y), 24), (y) ^= ROT((x), 16), (x) ^= (c)

// Inverse of 4-round ARX-box
#define ARXBOX_INV(x, y, c)                                             \
  (x) ^= (c), (y) ^= ROT((x), 16), (x) -= ROT((y), 24), (x) ^= (c),     \
      (y) ^= ROT((x), 31), (x) -= (y), (x) ^= (c), (y) ^= ROT((x), 17), \
      (x) -= ROT((y), 17), (x) ^= (c), (y) ^= ROT((x), 24),             \
      (x) -= ROT((y), 31)
...
void sparkle_ref(SparkleState *state, int brans, int steps) {
  ...
  for (i = 0; i < steps; i++) {
    ...
    state->y[1] ^= i;
    // ARXBox layer
    for (j = 0; j < brans; j++) ARXBOX(state->x[j], state->y[j], RCON[j]);
    // Linear layer
...
void sparkle_inv_ref(SparkleState *state, int brans, int steps) {
  ...
  for (i = steps - 1; i >= 0; i--) {
    ...
    // ARXbox layer
    for (j = 0; j < brans; j++) ARXBOX_INV(state->x[j], state->y[j], RCON[j]);
    // Add step counter
    state->y[1] ^= i;
    state->y[0] ^= RCON[i % MAX_BRANCHES];
  }
...
void print_state_ref(const SparkleState *state, int brans) {
  ...
  for (i = 0; i < brans; i++) {
    j = 4 * i;
    printf("(%02x%02x%02x%02x %02x%02x%02x%02x)", xbytes[j], xbytes[j + 1],
           xbytes[j + 2], xbytes[j + 3], ybytes[j], ybytes[j + 1],
           ybytes[j + 2], ybytes[j + 3]);
    if (i < brans - 1) printf(" ");
  }
  printf("\n");
}
